...
def plot_decision_regions_epcc(X, y, head,centers,
                               binary=True,
                               inverted=True,
                               single_label=False,
                            feature_index=None,
    ...
    # x_min, x_max = (X[:, x_index].min(),
    #                 X[:, x_index].max())
    xdist = X[:, x_index].max() - X[:, x_index].min()
    x_min, x_max = (X[:, x_index].min() - xdist/zoom_factor,
                    X[:, x_index].max() + xdist/zoom_factor)
    if dim == 1:
        y_min, y_max = -1, 1
    else:
        # y_min, y_max = (X[:, y_index].min(),
        #                 X[:, y_index].max())
        ydist = X[:, y_index].max() - X[:, y_index].min()
        y_min, y_max = (X[:, y_index].min() - ydist/zoom_factor,
                        X[:, y_index].max() + ydist/zoom_factor)
    ...
    X_predict_tensor = torch.from_numpy(X_predict).type(torch.FloatTensor)
    # centers_tensor = X_predict_tensor[y==1].mean(0)
    centers_tensor = centers
    Z = head.predict(X_predict_tensor.cuda(),centers_tensor.cuda()).cpu().detach().numpy()
    if(binary):
        if(inverted):
            Z = np.where(Z<=0,1,0)
        else:
...
def plot_decision_regions_feats(X, y,
                            feature_index=None,
                            filler_feature_values=None,
                            filler_feature_ranges=None,
                            ax=None,
