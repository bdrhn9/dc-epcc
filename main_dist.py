...
def train_one_epoch(backbone, head, criterion, criterion_center, optimizer, optimizer_center, data_loader, device, epoch, args):
    backbone.train(), head.train(), criterion.train(), criterion_center.train()
    ...
    for image, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        center_reg_loss = criterion_center(features,target)
        ...
        metric_logger.update(loss=loss.item(), gama_reg_loss=gama_reg_loss.item(), center_reg_loss = center_reg_loss.item(),lr=optimizer.param_groups[0]["lr"])
...
def main(args):
    ...
    backbone = IR_50(pretrained=True,input_size=[112,112])
    #model = resnet50(pretrained=True,input_size=112,vis2d=False)
    backbone.to(device)

    head = DC_EPCC(backbone.embed_size,len(dataset.classes),kapa=args.kapa)
    head.to(device)

    criterion = hinge_mc_v3(args.margin)
    criterion.to(device)

    criterion_center = center_regressor(backbone.embed_size,len(dataset.classes),mutex_label=False)
    ...
    if args.resume:
        ...
        backbone_without_ddp.load_state_dict(checkpoint['backbone'])
    ...
    if args.test_only:
        print(perform_val(device, 512, 128, backbone, lfw, lfw_issame,normalized=True))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, loss = train_one_epoch(backbone, optimizer, criterion_center, optimizer_center, data_loader, device, epoch, dataset.num_subset, dataset.num_subcluster, args)
        ...
        if(epoch%args.eval_freq == 0 and epoch!=0):
            ...
            val_acc1_norm_lfw, _best_threshold_norm_lfw, _roc_curve_norm_lfw = perform_val(device, 512, 128, backbone, lfw, lfw_issame,normalized=True)
            ...
            val_acc1_lfw, _best_threshold_lfw, _roc_curve_lfw = perform_val(device, 512, 128, backbone, lfw, lfw_issame,normalized=False)
            ...
            checkpoint = {
                'backbone': backbone_without_ddp.state_dict(),
...
def parse_args():
    ...
    parser.add_argument('--margin', default=1.0, type=float, help='decrease lr by a factor of lr-gamma')
